...
async function handleRecordSubmit() {
    ...
    try {
        ...
        console.error(err);
        alert("Âª∫Á?Á¥Ä?ÑÂ§±?óÔ?Ë´ãÁ?ÂæåÂ?Ë©?);
    }
}
...
function renderRecords() {
    ...
    list.forEach(r => {
        ...
        const summary = r.summary ? r.summary : "?°Ê?Ë¶ÅË?Ë®?;
...
function renderTodayOverview() {
    ...
    if (countEl) countEl.textContent = `${todayRecords.length} Á≠Ü`;
...
function renderSpotlight() {
    ...
    if (!tradeRecords.length) {
        ...
        card.innerHTML = '<div class="spotlight-empty">?´ÁÑ°Á¥Ä??/div>';
        card.onclick = null;
    ...
    const image = latest.image
        ...
        : '<div class="spotlight-empty">?°Â???/div>';
...
function showTimelineDetails(item) {
    ...
    if (!item || !item.list.length) {
        details.innerHTML = '<div class="activity-card">?∂Êó•?°‰∫§??/div>';
        return;
    }
    details.innerHTML = "";
...
function renderBestWorst() {
    ...
    const renderList = (container, list) => {
        container.innerHTML = "";
        if (!list.length) {
            container.innerHTML = '<div class="result-item text-gray-400 cursor-default">?´ÁÑ°Ë≥áÊ?</div>';
            return;
        }
        list.forEach(record => {
            const side = SIDE_LABELS[normalizeSide(record.side)] || record.side || "";
